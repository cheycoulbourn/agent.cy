...
- Platforms replaces Post versions. Creators can manually add any eligible platform not already attached to the brief, then edit its caption, opening adjustment, title where relevant, CTA, edit notes, posting target, and status independently. A confirmed delete removes only that platform and recalculates the master lifecycle.
