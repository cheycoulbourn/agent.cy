...
| `PERF-RISK-02` | `PAGE-ROOT-05`, `PAGE-ROOT-07`, `PAGE-MAIN-01`, `PAGE-MAIN-06`, `PAGE-CAP-02`, `PAGE-PLAN-01`, `PAGE-WORK-02` | Several state-heavy SwiftUI owners are 1,600 to 5,300 lines and combine many queries, derived collections, sheets, and navigation paths. PAGE-WORK-02 now buffers text writes and predicates its largest brief/workspace/series observers, but still owns eleven root queries and many sheets. | Count body updates and query invalidations during typing, tab changes, scrolling, and sheet presentation; profile hitches before further refactoring. |
...
| `PERF-RISK-11` | `PAGE-MAIN-03`, `PAGE-TASK-01`, `PAGE-TASK-02` | The Tasks root still holds six whole-table queries. PAGE-MAIN-03 removed duplicate full projection work and per-task archive construction; PAGE-TASK-01 now fetches its routed task, subtasks, linked outputs, and linked brief with bounded predicates instead of observing/scanning each full table, but its pillar/workspace menus remain whole-table observers. PAGE-TASK-02 now resolves workspace scope once and uses duplicate-safe indexes in one chooser projection instead of repeated nested scans, but its sheet still observes four whole tables. No Release-device measurement exists. | Profile a Release device with production-sized small, medium, and large workspaces. Measure body invalidations, SwiftData fetches, filter/group and chooser projection time, detail/composer presentation and save latency, task-toggle redraw, scrolling/frame hitches, retained-tab background work, and memory before splitting owners or adding indexes. |
...
- `PAGE-WORK-01` is `Confirmed defect (fixed)` + `Coverage gap`: Idea Draft is now a bounded local editor; implicit output creation, lifecycle downgrade, partial saves, missing archive, stale routing, and inherited post-editor weight are fixed. Eight focused regressions, 250 neighboring tests, phone replay, app/extensions build, and Catalyst build pass. See [`docs/audits/PAGE-WORK-01.md`](audits/PAGE-WORK-01.md).
- `PAGE-WORK-02` is `Confirmed defect (fixed)` + `Coverage gap`: lifecycle downgrade, false-success and partial saves, cross-output agenda corruption, handoff reversal, duplicate exit persistence, per-keystroke model writes, and unscoped large observers are fixed or bounded. Seven focused regressions, 305 neighboring tests, phone fixture replay, app/extensions build, and Catalyst build pass. Release-device profiling and the named driven/accessibility/real-store states remain. See [`docs/audits/PAGE-WORK-02.md`](audits/PAGE-WORK-02.md).
