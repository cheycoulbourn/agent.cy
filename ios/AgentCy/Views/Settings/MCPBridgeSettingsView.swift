...
struct MCPSeriesEpisodeReviewEditor: View {
    @Environment(\.dismiss) private var dismiss
    ...
    private var selectedPlatform: CreatorPlatform {
        payload.platform.flatMap(CreatorPlatform.init(rawValue:)) ?? .youtubeVideo
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: AgentSpacing.x8) {
                VStack(alignment: .leading, spacing: AgentSpacing.x3) {
                    MetaLabel("EPISODE \(payload.episodeNumber ?? 1)")
                    TextField("Episode title", text: textBinding(\.title), axis: .vertical)
                        .font(.paperInter(size: 28, weight: .bold, relativeTo: .title))
                        .tracking(-0.56)
                        .lineLimit(1...3)
                }
                ...
                VStack(spacing: 0) {
                    Stepper(value: episodeNumberBinding, in: 1...999) {
                        editorRow(label: "Episode", value: "\(payload.episodeNumber ?? 1)")
                    }
                    Divider().padding(.horizontal, AgentSpacing.x4)
                    TextField("Episode label", text: textBinding(\.episodeLabel))
                        .font(.agentBody)
                        .padding(.horizontal, AgentSpacing.x4)
                        .frame(minHeight: 54)
                ...
                dateEditor(
                    title: "Work date",
                    date: workDateBinding,
                    includesTime: includesWorkTimeBinding
                )

                VStack(alignment: .leading, spacing: 0) {
                    Toggle("Schedule this episode", isOn: scheduledBinding)
                        .font(.agentBody.weight(.semibold))
                        .padding(.horizontal, AgentSpacing.x4)
                        .frame(minHeight: 54)
                    if payload.targetDate != nil {
                        Divider().padding(.horizontal, AgentSpacing.x4)
                        DatePicker("Publish date", selection: targetDateBinding, displayedComponents: .date)
                            .font(.agentBody)
                            .padding(.horizontal, AgentSpacing.x4)
                            .frame(minHeight: 54)
                        Toggle("Include a publish time", isOn: includesTargetTimeBinding)
                            .font(.agentBody)
                            .padding(.horizontal, AgentSpacing.x4)
                            .frame(minHeight: 54)
                        if payload.includesTargetTime == true {
                            DatePicker("Publish time", selection: targetDateBinding, displayedComponents: .hourAndMinute)
                                .font(.agentBody)
                                .padding(.horizontal, AgentSpacing.x4)
                                .frame(minHeight: 54)
                        }
    ...
    private func editorRow(label: String, value: String) -> some View {
        HStack(spacing: AgentSpacing.x3) {
            MetaLabel(label)
            Spacer()
            Text(value)
    ...
    private func dateEditor(
        ...
        VStack(spacing: 0) {
            DatePicker(title, selection: date, displayedComponents: .date)
                ...
                .frame(minHeight: 54)
            Divider().padding(.horizontal, AgentSpacing.x4)
            Toggle("Include a work time", isOn: includesTime)
                .font(.agentBody)
                .padding(.horizontal, AgentSpacing.x4)
                .frame(minHeight: 54)
            if payload.includesWorkTime == true {
                DatePicker("Work time", selection: date, displayedComponents: .hourAndMinute)
                    .font(.agentBody)
                    .padding(.horizontal, AgentSpacing.x4)
                    .frame(minHeight: 54)
            }
    ...
    private var episodeNumberBinding: Binding<Int> {
        Binding(
            get: { payload.episodeNumber ?? 1 },
            set: { payload.episodeNumber = $0 }
        )
    }

    private var workDateBinding: Binding<Date> {
        Binding(
            get: { payload.workDate ?? Date() },
            set: { payload.workDate = $0 }
        )
    }

    private var includesWorkTimeBinding: Binding<Bool> {
        Binding(
            get: { payload.includesWorkTime ?? false },
            set: { payload.includesWorkTime = $0 }
        )
    }
    ...
    private var scheduledBinding: Binding<Bool> {
        Binding(
            get: { payload.targetDate != nil },
            set: { isScheduled in
                payload.targetDate = isScheduled ? (payload.targetDate ?? payload.workDate ?? Date()) : nil
                if !isScheduled { payload.includesTargetTime = false }
            }
        )
    }
    ...
    private var targetDateBinding: Binding<Date> {
        Binding(
            get: { payload.targetDate ?? payload.workDate ?? Date() },
            set: { payload.targetDate = $0 }
        )
    }
